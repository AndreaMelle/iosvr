...
namespace iosvr
{
    ...
    {
        if ([(CMMotionManager*)_manager isMagnetometerAvailable]
            && ![(CMMotionManager*)_manager isMagnetometerActive])
        {
            [(CMMotionManager*)_manager setMagnetometerUpdateInterval:(1.0f / 100.0f)];
            NSOperationQueue *magnetometerQueue = [[NSOperationQueue alloc] init];
            [(CMMotionManager*)_manager startMagnetometerUpdatesToQueue:magnetometerQueue
    ...
    {
        [(CMMotionManager*)_manager stopMagnetometerUpdates];
    }
    ...
    void MagnetSensor::addObserver(MagnetSensorObserver* observer)
    {
        std::vector<MagnetSensorObserver*>::iterator it = std::find(observers.begin(), observers.end(), observer);
        if(it == observers.end())
        {
    ...
    void MagnetSensor::removeObserver(MagnetSensorObserver* observer)
    {
        std::vector<MagnetSensorObserver*>::iterator it = std::find(observers.begin(), observers.end(), observer);
        if(it != observers.end())
        {
    ...
    void MagnetSensor::notifyObservers()
    {
        std::vector<MagnetSensorObserver*>::iterator it;
        for(it = observers.begin(); it != observers.end(); ++it)
        {
            (*it)->OnMagnetTrigger(this);
        }
    }
    
    void MagnetSensor::removeAllObservers()
    {
        observers.clear();
    }
